...
function configureOrtEnv(numThreads = 1, executionProviders = ['wasm']) {
  ...
  try {
    // 配置 WASM 文件路径 - 重要！
    // 在 Chrome 扩展的 Worker 中，需要使用绝对路径
    const baseUrl = self.location.origin;
    ort.env.wasm.wasmPaths = {
      'ort-wasm.wasm': `${baseUrl}/libs/ort-wasm.wasm`,
      'ort-wasm-simd.wasm': `${baseUrl}/libs/ort-wasm-simd.wasm`,
      'ort-wasm-threaded.wasm': `${baseUrl}/libs/ort-wasm-threaded.wasm`,
      'ort-wasm-simd-threaded.wasm': `${baseUrl}/libs/ort-wasm-simd-threaded.wasm`,
    };

    // 强制使用单线程以避免CSP问题
    // 多线程WASM会创建blob URL的Worker，违反Chrome扩展CSP
    ...
    // 过滤执行提供者，避免多线程WASM
    const filteredProviders = executionProviders.filter(
      provider => provider !== 'wasm-threaded' && provider !== 'wasm-simd-threaded',
    );

    sessionOptions = {
      executionProviders: filteredProviders.length > 0 ? filteredProviders : ['wasm'],
      graphOptimizationLevel: 'all',
      enableCpuMemArena: true,
      enableMemPattern: true,
      executionMode: 'sequential', // 强制顺序执行，避免多线程
    };
  } catch (error) {
...
async function initializeModel(modelPathOrData, numThreads, executionProviders) {
  try {
    ...
    if (modelPathOrData instanceof ArrayBuffer) {
      console.log(`Worker: Initializing model from cached ArrayBuffer (${modelPathOrData.byteLength} bytes)`);
...
async function runBatchInference(batchData) {
  ...
  try {
    ...
    feeds['input_ids'] = new ort.Tensor('int64', inputIdsBuffer.slice(0, inputIdsLength), batchData.dims.input_ids);
...
async function runInference(inputData) {
  ...
  try {
    ...
    feeds['input_ids'] = new ort.Tensor('int64', inputIdsBuffer.slice(0, inputIdsLength), inputData.dims.input_ids);
    feeds['attention_mask'] = new ort.Tensor(
      'int64',
      attentionMaskBuffer.slice(0, attentionMaskLength),
