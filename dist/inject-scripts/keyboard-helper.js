  ...
  function parseSingleKeyCombination(keyString) {
    ...
    if (!mainKeyPart) {
      ...
      if (Object.keys(MODIFIER_KEYS).includes(parts[parts.length - 1]) && parts.length === 1) {
        ...
        if (mainKeyPart === 'shift') return { key: 'Shift', code: 'ShiftLeft', keyCode: 16, modifiers };
    ...
    try {
      ...
      for (let i = 0; i < keyCombinations.length; i++) {
        ...
        if (dispatchResult.error) {
          ...
          console.warn(`Failed to simulate key combination "${comboString}": ${dispatchResult.error}`);
        }
