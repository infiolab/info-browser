...
if (window.__WEB_FETCHER_HELPER_INITIALIZED__) {
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    const pingActions = ['chrome_search_tabs_content_ping', 'chrome_web_fetcher_ping', 'chrome_get_web_content_ping'];
  ...
  function extractPageMetadata() {
    ...
    // Extract author
    const authorElement = document.querySelector('meta[name="author"], meta[property="article:author"]');
    if (authorElement) {
    ...
    const publishedElement = document.querySelector('meta[property="article:published_time"], time[datetime]');
    if (publishedElement) {
      metadata.published = publishedElement.getAttribute('content') || publishedElement.getAttribute('datetime') || '';
    }
  ...
  function extractIframeContent() {
    ...
    for (const iframe of iframes) {
      try {
        ...
        console.warn(`Cannot access iframe content (possible cross-origin restriction): ${error.message}`);
      }
    }
  ...
  function cleanHtmlContent(html) {
    ...
    const svgImages = doc.querySelectorAll('img[src$=".svg"], object[data$=".svg"], embed[src$=".svg"]');
