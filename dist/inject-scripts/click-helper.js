...
if (window.__CLICK_HELPER_INITIALIZED__) {
   ...
   */
  async function clickElement(selector, waitForNavigation = false, timeout = 5000, coordinates = null) {
    try {
      let element = null;
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    if (request.action === 'clickElement') {
      clickElement(request.selector, request.waitForNavigation, request.timeout, request.coordinates)
        .then(sendResponse)
        .catch(error => {
          sendResponse({
